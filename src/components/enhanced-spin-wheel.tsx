...
const probabilityDistribution = [
  454, // ₹500 off (index 1)
  300, // ₹1000 off (index 3)
  100, // Try Again (index 4)
  46,  // ₹2500 off (index 0)
];
...
export default function EnhancedSpinWheel() {
  ...
  const handleSpinClick = () => {
    if (!mustSpin) {
      const totalProbability = probabilityDistribution.reduce((a, b) => a + b, 0);
      ...
      // Map the selected index to the correct prize number
      const prizeMap = [1, 3, 4, 0]; // Maps probability index to data index
      const newPrizeNumber = prizeMap[selectedIndex];
